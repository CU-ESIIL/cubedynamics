...
def build_story() -> list:
    ...
    story.append(
        p(
            "Wildfire events are usually compared by final area, perimeter, duration, or daily growth, even though impact and management opportunity depend on how a fire develops through time. We introduce Fire VASE, a geometry-first representation that converts each observed fire history into a comparable developmental object. Applied to 278,569 FIRED events from 2000-2021, Fire VASE defines a reproducible low-dimensional coordinate system: five geometry-only axes explain 96.3% of standardized feature variance, with PC1 explaining 81.0%. Real-fire medoids reveal recurring developmental neighborhoods arranged along continuous gradients rather than hard classes. New null and sensitivity analyses show that this low dimensionality is robust but partly shaped by cumulative-profile redundancy and the prevalence of short fires, so we do not claim that observed fires occupy a restricted subset of all plausible trajectories. Projecting gridMET climate onto 237,235 climate-complete fires shows that temperature, vapor pressure deficit, and wind align with morphology, but blocked validation and matched examples show that climate and morphology are not interchangeable. Fire VASE is therefore best understood as a coordinate system for organizing fire development before richer perimeter-based climate, fuel, topographic, and management explanations are added.",
            st["abstract"],
        )
    )
    ...
    add_section(
        ...
        "Introduction",
        [
            "Wildfire science now measures fire at continental scales. Satellite burned-area products delineate events, event-building algorithms reconstruct daily histories, and gridded meteorological products describe the atmospheric context in which those events unfold [1-3]. Those tools have helped show that fire spread and area burned depend on weather, vapor pressure deficit, fuels, topography, antecedent aridity, ignition context, and human intervention [4-9].",
    ...
    add_subsection(
        story,
        "Observed Fire Histories Occupy A Reproducible Low-Dimensional Coordinate System",
        [
    ...
    add_subsection(
        ...
        [
            "The audit analyses replace the older axis-interpretation figure with a direct test of how much of the dominant structure survives feature choices and null assumptions (Fig. 3). PC1 remains large after scale and duration variables are removed, and the first five axes remain high-variance under several feature subsets. This indicates that the coordinate system is not simply a final-size or duration ranking.",
            "The same analyses also show why stronger constraint language should be avoided. Profile-only and growth-share-only feature sets are extremely low dimensional because their encodings are themselves redundant. Null developmental universes that preserve duration, final area, zero-growth frequency, empirical increments, or duration-bin mean profiles often approach the observed first-five-axis variance. Support-overlap diagnostics further show that the null populations are not cleanly separated from observed fires in PC1-PC5 space. Figure 3 therefore turns an overstrong claim into a more useful result: PC1 is a reproducible developmental-profile axis whose biological interpretation requires stricter future nulls and richer covariates.",
        ],
        st,
    )
    ...
    add_subsection(
        ...
        [
            ...
            "Directional climate-morphology association was evaluated with held-out linear baseline models under random, region-blocked, and year-blocked folds. The models are written informally as climate predicting morphology and morphology predicting climate, but they are not causal estimates. Partial-history prediction is retained as Supplementary Figure 2 using fixed observed-day stages and predictors available by that day only. The older fractional-stage table is excluded from affirmative prediction claims because several variables were normalized by final fire size or counted future pulses.",
        ],
        st,
    )
    ...
    refs = [
        ...
        "Abatzoglou, J. T. and Williams, A. P. Impact of anthropogenic climate change on wildfire across western US forests. Proceedings of the National Academy of Sciences 113, 11770-11775 (2016).",
        ...
        "Holsinger, L. M., Parks, S. A. and Miller, C. Weather, fuels, and topography impede wildland fire spread in western US landscapes. Forest Ecology and Management 380, 59-69 (2016).",
        ...
        "Mitteroecker, P. and Gunz, P. Advances in geometric morphometrics. Evolutionary Biology 36, 235-247 (2009).",
    ]
