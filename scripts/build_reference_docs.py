...
from reference_classification import CATEGORIES, COMPATIBILITY, KIND_LABELS, classify
...
def vignette_links(page, name, reference_path=None):
    found = []
    for path in sorted(DOCS.glob("**/*.ipynb")):
        ...
        if re.search(rf"\b(?:v|data)\.{re.escape(name)}\s*\(", code) or (reference_path and reference_path in prose):
...
def generate():
    ...
    for noun, sources in catalog.items():
        ...
        if len(entries) > 1:
            text += section("Differences among source flavors",
                "The same noun does not make these products numerically interchangeable. "
                "CubeDynamics does not silently harmonize units, grids, statistics, or revisions.\n\n" +
                table(["Property", *[e['source'] for e in entries]], [
    ...
    def callable_rows(names):
        return [(f"[{n}]({n}.md)", KIND_LABELS[classification[n]['kind']],
                 classification[n]['description'] or summary(verbs[n])) for n in names]

    verb_index = "# Verbs by purpose\n\nBrowse implemented operations by what they do. "
    verb_index += "Grammar verbs configure pipe stages; direct helpers take their data arguments immediately. "
    verb_index += "A shared namespace does not make every function a verb factory.\n\n"
    verb_index += "[All public callables (A–Z)](a-z.md) · [Learn: verbs](../../learn/verbs.md)\n\n"
    groups = [*CATEGORIES, "Other helpers"]
    verb_index += " · ".join(f"[{g}](#{g.lower().replace(' ', '-')})" for g in groups) + "\n"
    for group in groups:
        names = [n for n, c in classification.items() if c['status'] == 'implemented' and c['category'] == group]
        if names:
            verb_index += section(group, table(["Operation", "Callable type", "Purpose"], callable_rows(names)))
    verb_index += section("Other public names", "[Compatibility and deprecated helpers](compatibility.md) · [Reserved / planned API](planned.md)\n\nThese remain documented but are not promoted as new analysis operations.")
    pages["reference/verbs/index.md"] = verb_index
    pages["reference/verbs/a-z.md"] = "# All public callables (A–Z)\n\nComplete inventory of the public `cubedynamics.verbs` namespace. " + "[Browse implemented operations by purpose](index.md).\n\n" + table(
        ["Name", "Callable type", "Availability", "Meaning"],
        [(f"[{n}]({n}.md)", KIND_LABELS[c['kind']], c['status'], c['description'] or summary(verbs[n])) for n, c in classification.items()])
    ...
    for name, func in verbs.items():
        ...
        browse = "planned.md" if info['status'] == 'placeholder' else "compatibility.md" if info['status'] in {'compatibility', 'deprecated'} else "index.md#" + info['category'].lower().replace(' ', '-')
        text += f"\n**Callable type:** {KIND_LABELS[info['kind']]} · [Browse: {info['category'] if info['status'] == 'implemented' else info['status']}]({browse})\n"
        if info['kind'] in {'helper', 'visualization_helper'}:
        ...
        if info['status'] in {'compatibility', 'deprecated'}:
            text += f"\n> **{'Deprecated' if info['status'] == 'deprecated' else 'Compatibility / legacy'}:** {COMPATIBILITY.get(name, summary(func))}\n"
        if note.get("status"):
        ...
        related = vignette_links(page, name) + [link(page, "learn/verbs.md", "Learn: verbs"), link(page, "library/index.md", "Noun library"), link(page, "reference/verbs/index.md", "Verbs by purpose"), link(page, "reference/verbs/a-z.md", "All public callables (A–Z)")]
