...
def build(artifact_dir: Path, output_pdf: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="surface-atlas-") as directory:
        ...
        story = []
        _add_page(story, styles, "1 · Colorado local synchrony surfaces", "Revised Phase 2 · observed PRISM · 1 Nov 2023–30 Jan 2024", image=images["pair"], paragraphs=["The local 2-D surface S_p(dx,dy) is the scientific object. Distance and direction are coordinates. Each canonical pair contributes to both endpoint surfaces with opposite signed displacement and bearing. The 100 km limit is an observation window, not an inferred synchrony scale."])
        _add_page(story, styles, "2 · Overlapping center maps generate S_p(dx,dy)", image=images["overlap"])
        ...
        _add_page(story, styles, "6 · Continuous radial profiles", image=images["radial"], paragraphs=["Five-kilometer annuli are numerical support for S_p(r), not prescribed scientific scales."])
        ...
        _add_page(story, styles, "22 · Landscape change is separate", image=images["landscape"], paragraphs=["This branch compares complete center landscapes M_i and M_j; it does not describe within-surface shape."])
