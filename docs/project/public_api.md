...
- Synchrony grammar verbs include state constructors (`v.threshold_state`, `v.quantile_state`, `v.binary_state`, `v.change_state`), local-cell event detection (`v.detect_events`), explicit regional episode consolidation (`v.consolidate_events`), bounded period summaries (`v.event_metrics`), and primitive synchrony operators (`v.occurrence_synchrony`, `v.severity_synchrony`, `v.timing_synchrony`, `v.duration_synchrony`). The climate-tail relationship branch measures sparse canonical pairs (`v.local_synchrony_pairs`), reconstructs local relational geography (`v.local_synchrony_surface`), describes candidate surface structure (`v.synchrony_surface_diagnostics`), reduces within declared fixed supports (`v.synchrony_signature`), diagnoses finite-range resolvability (`v.empirical_synchrony_range`), characterizes continuous decay (`v.empirical_synchrony_decay`), and compares complete center landscapes (`v.landscape_change_signature`). `v.adaptive_synchrony_experiment` remains a public staged first-break experiment whose current evidence gate is HOLD, not a recommended production radius selector. Temporal-support review (`v.align_time`), biological cube helpers (`v.rasterize_observations`, `v.align_cube`), and same-pixel lagged coupling (`v.sync_with`) remain separate. Pair synchrony, local surface, center landscape, reduced focal summary, landscape change, discovery support, finite range, fractional decay, effective length, and local decay rate are distinct scientific objects. These verbs are public but intentionally narrow; event-relative trajectories, episode-level synchrony, richer null diagnostics, historical-tail definitions, and classification are future extensions.
- `cubedynamics.version_info()` and `RuntimeInfo` are public runtime-inspection helpers. They report local import/distribution/VCS identity without network access and do not replace release manifests or scientific provenance.
