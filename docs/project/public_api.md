...
- Synchrony grammar verbs include state constructors (`v.threshold_state`, `v.quantile_state`, `v.binary_state`, `v.change_state`), local-cell event detection (`v.detect_events`), explicit regional episode consolidation (`v.consolidate_events`), bounded period summaries (`v.event_metrics`), primitive synchrony operators (`v.occurrence_synchrony`, `v.severity_synchrony`, `v.timing_synchrony`, `v.duration_synchrony`), sparse local relationship construction (`v.local_synchrony_pairs`), kernel-agnostic empirical range reduction (`v.empirical_synchrony_range`), empirical decay characterization (`v.empirical_synchrony_decay`), temporal-support review (`v.align_time`), biological cube helpers (`v.rasterize_observations`, `v.align_cube`), and same-pixel lagged coupling (`v.sync_with`). Event scope, event-label matching, observation-support alignment, discovery support, resolved synchrony range, fractional decay, effective length, and local decay rate remain separate. These are public but intentionally narrow in their first implementation: event-relative trajectories, episode-level synchrony, richer null diagnostics, and classification are future extensions.
