...
| Units by source | gridmet: default: kPa |
...
```python
data.vpd(*, source='gridmet', bbox=None, geometry=None, aoi_geojson=None, lat=None, lon=None, start=None, end=None, **kwargs)
```
...
| --- | --- | --- | --- | --- |
| [gridmet](../sources/gridmet.md) | Contiguous United States | 4,638.3 m | 1979-present; daily mean | PRISM VPD is not registered until its daily NcSS path is validated. |

## Returned data

An `xarray.DataArray` named `vpd`, normally with `time, y, x` dimensions. `time` stores acquisition/observation labels; `y` and `x` store grid coordinates in the declared CRS. Inspect the actual dimensions and CRS before combining sources.
...
| --- | --- | --- | --- | --- | --- |
| gridmet | implemented | vpd.gridmet@2026-08-26.1 | VALIDATED | STALE | climate_continuous_daily |
