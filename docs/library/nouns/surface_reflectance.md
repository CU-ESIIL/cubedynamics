...
| --- | --- | --- | --- | --- |
| [sentinel2](../sources/sentinel2.md) | Global land surfaces | 10, 20, or 60 m by band; CubeDynamics default 10 m | 2015-present; scene acquisition; nominal five-day constellation revisit | Scene-level cloud filtering does not remove every cloudy pixel. |

## Returned data

An `xarray.DataArray` named `surface_reflectance`, normally with `time, band, y, x` dimensions. `time` stores acquisition/observation labels; `y` and `x` store grid coordinates in the declared CRS. Inspect the actual dimensions and CRS before combining sources.
