...
def test_ci_upgrades_the_fresh_environments_pip_before_installing_wheel(workflow, environment):
    ...
    expected_install = (
        f'"$RUNNER_TEMP/{environment}/bin/python" -m pip install "release-bundle/$WHEEL"'
        if workflow == "publish.yml"
        else f'"$RUNNER_TEMP/{environment}/bin/python" -m pip install "dist/$WHEEL"'
    )
    install = commands.index(expected_install)
